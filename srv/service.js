...
class InsentiveCommision extends cds.ApplicationService {
  init() {
    ...
    /*--------------------------------------------------------------Action for Insentive calculation----------------------------------------------------------------------------*/
    var d = new Date();
    var dt =
      d.getFullYear() +
      "-" +
    ...
    this.on("calculatedata", async (req) => {
      try {
        ...
        var query =
          "SELECT COUNT(1) as CS FROM ICM_TRANS_CAL_DATA WHERE SCHEMEID = ? AND CREATEDDATE = ? AND PERCENTILE = ?";
        //console.log(query);
        const sp1 = await dbConn.preparePromisified(query);
        var SchemeData = await dbConn.statementExecPromisified(sp1, [
          req.data.schemeid,
          dt,
          req.data.percentile,
        ]);
        ...
        if (cnt === 0) {
          ...
          const query2 = await db.run(
            "SELECT TOP 1 (ROWSID + 1) AS ROWSID FROM ICM_TRANS_CAL_DATA WHERE ISDEL = 0 ORDER BY CID DESC",
            []
          );
          var rowid = query2[0]["ROWSID"];
        }

        var statementpreparation =
          " SELECT SKUID,REGIONCODE,SALES_AGENT, DAILY_SALES ,SALE_DATE,SUM(DAILY_SALES) OVER (PARTITION BY SALES_AGENT ORDER BY SALE_DATE,SKUID,REGIONCODE,SALES_AGENT) AS TOTAL_SALES , " +
        ...
        if (
          req.data.skuid !== null &&
          req.data.skuid !== undefined &&
          req.data.skuid !== ""
        ) {
          statementpreparation += "AND SKUID IN ('" + req.data.skuid + "') ";
        }
        if (
          req.data.regioncode !== null &&
          req.data.regioncode !== undefined &&
          req.data.regioncode !== ""
        ) {
          statementpreparation +=
            "AND REGIONCODE IN ('" + req.data.regioncode + "') ";
        }
        if (
          req.data.empcode !== null &&
          req.data.empcode !== undefined &&
          req.data.empcode !== ""
        ) {
          statementpreparation +=
            "AND SALES_AGENT IN ('" + req.data.empcode + "') ";
        }
        statementpreparation +=
          "AND SALE_DATE BETWEEN ? AND ? GROUP BY SKUID,REGIONCODE,SALES_AGENT,SALE_DATE ORDER BY SALE_DATE ASC " +
        ...
        if (req.data.to_date < dt) {
          SalesData = await dbConn.statementExecPromisified(sp, [
            req.data.minsales,
            req.data.percentile,
            req.data.from_date,
            req.data.to_date,
        ...
        const sQuery1 = await db.run(
          'SELECT IFNULL(MAX("JOBID"), 0) + 1 AS JOBID FROM ICM_TRANS_CAL_DATA',
          []
        );
        ...
        if (SalesTransData.length > 0) {
          for (let j = 0; j < SalesTransData.length; j++) {
            ...
            const db = await cds.connect.to("db");
<<<<<<< HEAD
            const sQuery = await db.run(
              'CALL "prCreateTransIncentiveData" (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
              [
